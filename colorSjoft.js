...
function cssToHSL(cssColor) {
  // Extracting HSL values from the CSS color string
  const match = cssColor.match(/hsl\((\d+),\s*([\d.]+)%,\s*([\d.]+)%\)/);
  if (!match) {
    throw new Error("Invalid CSS color string");
  }

  // Extracted HSL values
...
styles = Object.fromEntries(
  Object.entries(styles).map(([cls, color]) => [cls, hexToHSL(color)])
);
